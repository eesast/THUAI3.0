...
namespace Timer
{
    class Program
    {
        ...
        public static void Say2(object a)
        {
    ...
    public class Timer
    {
        ...
        DateTime endTime;
        public Timer(TimerCallback callback_t, object state_t, int dueTime_t, int period_t)
        {
            void callback(object state)
            {
                endTime += TimeSpan.FromMilliseconds(period_t);
                callback_t(state);
            }
            myTimer = new System.Threading.Timer(callback, state_t, dueTime_t, period_t);
            endTime = DateTime.Now + TimeSpan.FromMilliseconds(dueTime_t);
        }
        ...
        public TimeSpan getLeftTime()//输出并返回下一个回调函数开始执行的时间
        {
            if (DateTime.Now < endTime)
                Console.WriteLine("\b \b距离下次回调还有" + (endTime - DateTime.Now));
            else Console.WriteLine("计时器没有在运行");
    ...
    public class Timers
    {
        ...
        public void threadFunctionType1()//该函数使得各回调函数的开始时间间隔保持一致，而非一个执行完另一个再开始计时
        {
            for (int i = 0; i < count; i++)
            {
                ...
                flag++;
                endTime = Program.getGameTime() + TimeSpan.FromSeconds(dueTime[i]);
        ...
        public void threadFunctionType2(object j)//上一个计时器的回调函数执行完之后再开始下一个计时器的计时
        {
            void func(object k)
            {
                ...
                if (i != count - 1)
                {
                    endTime = Program.getGameTime() + TimeSpan.FromSeconds(dueTime[(int)k + 1]);flag++;
            ...
            endTime = Program.getGameTime() + TimeSpan.FromSeconds(dueTime[0]);flag++;
        ...
        public TimeSpan getLeftTime()//输出并返回下一个回调函数开始执行的时间
        {
            if (Program.getGameTime() < endTime) Console.WriteLine("\b \b距离下次回调还有" + (endTime - Program.getGameTime()) + " 当前正在进行第" + flag + "个委托");
            else Console.WriteLine("已经全部执行");
            return (endTime - Program.getGameTime());
        }
        public void check()//手动检查，按q退出计时器，其他键调用getLeftTime
        {
            char key;
